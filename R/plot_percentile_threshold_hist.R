#' Plot histograms with percentile thresholds
#'
#' @param values A data.frame with sampled raster values.
#' @param bands Character vector of band names.
#' @param params A data.frame with columns `lower_p`, `upper_p`, `lower`, and `upper`.
#' @param side Which threshold side is currently used.
#'
#' @return A ggplot object.
#' @export
plot_percentile_threshold_hist <- function(
  values,
  bands,
  params,
  ...
  if (!all(c("lower_p", "upper_p", "lower", "upper") %in% names(params))) {
    stop("`params` must contain columns `lower_p`, `upper_p`, `lower`, and `upper`.", call. = FALSE)
  }
